...
The smoke suite (`tests/smoke/`) starts the production server on an empty,
migrated database, creates the first user, the store settings and a product,
then walks the storefront and the admin. CI runs it on every push.
